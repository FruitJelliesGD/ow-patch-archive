...
- No longer allows storage of excess ultimate charge.
...
- No longer provides self-healing when healing an ally.
...
- Movement abilities no longer immediately start passive health regeneration.
...
- Fixed an issue where the display price for 1000 Overwatch Coins appeared incorrectly in some languages
- Fixed an issue causing the AFK timer to behave incorrectly on the Neon Junction map.
- Fixed an issue where Hero Bans would not show when the ban‑role limit was reached.
- Fixed an issue that prevented Shion's Burst from being used during Satsuriku Spree while hacked or mobility‑locked, despite being governed by Transformation logic.
- Fixed an issue which was causing our lighting to render some heroes brighter than intended.
