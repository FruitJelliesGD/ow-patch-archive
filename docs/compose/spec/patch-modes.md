...
**What was built** — 区分常规与非常规模式补丁（社区模式 Quick Play: Hacked、愚人节、6v6 实验、英雄试玩、PTR、公告），非常规数据**不再干扰常规历史**：英雄轨迹/词条/数值轨迹默认只显示常规、可切换查看非常规；首页时间浏览与补丁详情页列出全部补丁并带模式徽标。

- **标题规则**（新模块 `modes.py`，仅标题规则）：`Quick Play:? Hacked`/`Really, Really, Really Balanced|Totally Normal|完全正常`/`6v6 Experiment`/`Hero Trial|英雄试玩`/`\bPTR\b`/`Unauthorized Peripheral|Welcome to Overwatch` → 各模式；默认 standard；大小写变体（`OVERWATCH PATCH NOTES`）不误伤；**CN 愚人节关键词 `完全正常` 兜底**。
- **数据层**：mode 唯一权威 = patches_index **pair 级**（任一侧非 standard 即非 standard，en 优先）——`cn-2025-04-01-1`（中文标题 144 条记录）经 pair 兜底判为 april_fools；build_hero_files 反查 patch_id→mode 给每条时间线记录打标（**全部记录保留**供切换），**values 只喂常规记录**（非常规数值从不进入数值轨迹）；entries_index 常规口径（非常规-only 词条从检索消失）。实测：342/342 索引条目、10,788/10,788 时间线记录全部带 mode；**standard 10,153 + special 635**（april_fools 418 含 CN 144、experiment_6v6 113、hero_trial 104；quick_play_hacked/ptr/announcement 无 hero 结构 0 条）。
...
- **build_entries_index**（entries.py）：**常规口径**——只统计 `mode == "standard"` 记录（count/edited/first/last_date 基于常规）；非常规-only 词条（实证 31 个：29 ability/weapon + 1 hero_attr wrecking-ball::move_speed + 1 perk wuyang::falling-rain）从检索消失（常规口径的预期结果，entries 987→956）。
